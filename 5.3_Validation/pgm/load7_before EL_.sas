...
DATA _null_;
  %if %SYSEVALF(&sysver > 9) %then infile &f.file MISSOVER DSD LRECL=2048 firstobs=1 obs=1 TERMSTR=CRLF;
  %else infile &f.file MISSOVER DSD LRECL=2048 firstobs=1 obs=1;;

  file inpfile;
  input (varn1-varn280) ($);  /* varn250 => varn280 for X10 */
  array vars(*) $8 varn1--varn280; /* varn250 => varn280 for X10 */
