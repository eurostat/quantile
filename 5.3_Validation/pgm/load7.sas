...
%include "&eusilc/pgm/&ss&yy&f.var.0sas" /lrecl=3000;  *** definition of variables list; 
...
DATA _null_;
  %if %SYSEVALF(&sysver > 9) %then infile &f.file MISSOVER DSD LRECL=4096 firstobs=1 obs=1 TERMSTR=CRLF;
  %else infile &f.file MISSOVER DSD LRECL=4096 firstobs=1 obs=1;;

  file inpfile;
  input (varn1-varn350) ($);  /* varn250 => varn350 for R15 pfile (300 variables) */
  array vars(*) $8 varn1--varn350; /* varn250 => varn350 for R15 */
...
DATA dds.&ss&cc&yy&f (drop= LEN varx varrest);
  length txt $200; 
  retain missid 0 f_b020 0 f_db040 0 f_db041 0 f_pb210 0 f_pb220a 0 f_pb220b 0 f_db090 0 f_rb050 0 f_pb040 0 f_pb060 0; 
  %if %SYSEVALF(&sysver > 9) %then infile &f.file MISSOVER DSD LRECL=4096 firstobs=2 end=fin TERMSTR=CRLF;
  %else infile &f.file MISSOVER DSD LRECL=4096 firstobs=2 end=fin;;
  %include frmfile;
  ...
  /* change in DB020 RB020 HB020 PB020*/
	...
	/* change in DB040*/
		/* filtering cases when country code GR was not changed in EL and nuts10 codes not changed in nuts13 codes (FR and EL)*/
	%if &f=d and &RCC=EL %then %do;
			varx=substr(DB040,1, 2);
			varrest=substr(DB040,3, 2);
			IF substr(DB040,1, 2)= "GR" then do;
				varx="EL";
				f_db040 = f_db040 +1;
			END;	
			IF substr(DB040,3, 2)=11 then do;
				varrest=51;
				f_db041 = f_db041 +1;
			END;
			IF substr(DB040,3, 2)=12 then do;
				varrest=52;
				f_db041 = f_db041 +1;
			END;
			IF substr(DB040,3, 2)=13 then do;
				varrest=53;
				f_db041 = f_db041 +1;
			END;
			IF substr(DB040,3, 2)=14 then do;
				varrest=61;
				f_db041 = f_db041 +1;
			END;
			IF substr(DB040,3, 2)=21 then do;
				varrest=54;
				f_db041 = f_db041 +1;
			END;
			IF substr(DB040,3, 2)=22 then do;
				varrest=62;
				f_db041 = f_db041 +1;
			END;
			IF substr(DB040,3, 2)=23 then do;
				varrest=63;
				f_db041 = f_db041 +1;
			END;
			IF substr(DB040,3, 2)=24 then do;
				varrest=64;
				f_db041 = f_db041 +1;
			END;
			IF substr(DB040,3, 2)=25 then do;
				varrest=65;
				f_db041 = f_db041 +1;
			END;
			IF f_db040>0  OR f_db041>0 then do;
				DB040=cats(varx, varrest);
			END;
			
	%end;
	%if &f=d and &RCC=FR %then %do;
			
			IF DB040="FR92" then do;
				DB040="FRA2";
				f_db041 = f_db041 +1;
			END;
			IF DB040="FR93" then do;
				DB040="FRA3";
				f_db041 = f_db041 +1;
			END;
			IF DB040="FR94" then do;
				DB040="FRA4";
				f_db041 = f_db041 +1;
			END;			
...
if fin and f_db041 > 0 then do;
  	txt = "----> "||trim(put(f_db041,best6.))||" records with nuts10 instead of nuts13 in DB040";
	put "----> ";
	put txt;
end;
...
if fin and f_pb060 > 0 then do;
  ...
  drop txt missid f_b020 f_db040 f_db041 f_pb210 f_pb220a f_pb220b f_db090 f_rb050 f_pb040 f_pb060;
RUN;
