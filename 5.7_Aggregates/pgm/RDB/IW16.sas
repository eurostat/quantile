...
*** IN-WORK At-risk-poverty-rate by age and gender  and C_BIRTH  ***;

%macro UPD_iw16(yyyy,Ucc,Uccs,flag) /store;
...
CREATE TABLE missunrel AS SELECT mARPT60i.DB020,
                                 max(mARPT60i, mC_BIRTH ) AS pcmiss
	FROM mARPT60i
	LEFT JOIN mC_BIRTH  ON (mARPT60i.DB020 = mC_BIRTH.DB020)
	;;
QUIT;
...
%if &EU=0 %then %do;
	...
	TABLE DB020 * AGE * RB090 *C_BIRTH  , &arpt * PB040 * (RowPctSum N Sum) /printmiss;
