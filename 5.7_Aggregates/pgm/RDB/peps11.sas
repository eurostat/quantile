*** AROPE    by NUTS1 NUTS2 and COUNTRIES ***;
/****************************************************************************/
...
/*                                                                          */
/****************************************************************************/
%macro UPD_peps11(yyyy,Ucc,Uccs,flag,notBDB);
%global nat;
%let tab=peps11;
%let cc=%lowcase(&Ucc);
...
Create table work.idb as 
	select distinct DB010, DB020, RB030, RB050a, Age, RB090,   NUTS1, NUTS2, arope
...
Select distinct count(&nut) as N 
	into :nobs
	from  work.idb;
Create table work.peps11 like rdb.peps11; 
QUIT;
**** MISSINGS *;
...
CREATE TABLE missunrel AS SELECT m&NUT..DB020, 
	max(m&NUT, marope) AS pcmiss
	FROM m&NUT LEFT JOIN marope ON (m&NUT..DB020 = marope.DB020);
quit;
...
PROC TABULATE data=work.idb out=Ti;
   FORMAT AROPE $f_AROPE.;
	...
	CLASS DB020;
	VAR RB050a;
	TABLE DB020 * &nut, AROPE * RB050a * (RowPctSum N Sum) /printmiss;
RUN;
...
LEFT JOIN work.old_flag ON (Ti.&nut = old_flag.geo) AND (Ti.AROPE="1") 
...
/***  UPDATE RDB ***/
%if &nut=NUTS1 %then %do;
	DATA  rdb.peps11 (drop=AROPE);
					set rdb.peps11(where=(not(time = &yyyy and  substr(geo,1, 2)= "&Ucc" and length(geo)=3 )))
    				work.peps11; 
					where AROPE="1";
					run;  
	%end;	
%if &nut=NUTS2 %then %do;	
 %if (&yyyy =2007 or &yyyy =2008) and &Ucc=RO  %then %do;
    data work.peps11;  set work.peps11;  
...
%if &nut=DB020  %then %do;
DATA  rdb.peps11(drop=AROPE); ;
					set rdb.peps11(where=(not(time = &yyyy and  geo = "&Ucc" and length(geo)=2  )))
    				work.peps11; 
					where AROPE="1";
					run;  
%end; 
PROC SQL;  
     ...
     set date = "&sysdate"d, time = "&systime"t, user = "&sysuserid",
		 report = "* &nut - &Ucc - &yyyy * peps11 (re)calculated *";		  
QUIT;
%end;
%else %do; 
PROC SQL;  
     Insert into log.log
     set date = "&sysdate"d, time = "&systime"t, user = "&sysuserid",
		 report = "* &nut - &Ucc- &yyyy * peps11 NO DATA !";		  
QUIT;
%end;
...
%macro expcsv;
/* to export the xls file for now it is not called */
%if %sysfunc(exist(rdb.peps11, DATA)) %then %do;           
 %let fname1=/ec/prod/server/sas/0eusilc/IDB_RDB_TEST/RDB/nuts1.csv ;   
 %let fname2=/ec/prod/server/sas/0eusilc/IDB_RDB_TEST/RDB/nuts2.csv ; 
 
   		Data nuts1;
 		set  rdb.peps11(where=(length(geo)=3));run; 
 		proc export data=nuts1  outfile="&fname1 "
				dbms=dlm replace; 			
  		delimiter=',';                         
  		quit;	
		...
		Data nuts2;
		set  rdb.peps11(where=(length(geo)=4));run;
  		proc export data=work.nuts2 outfile="&fname2 "
				dbms=dlm replace; 			
    	delimiter=',';                         
  		quit;	
		%sysexec chmod 777  $fname2  ; 
%end;
%mend expcsv;
/* end macro */
...
%let nut=%scan(&listnut,&i,%str( )); 
  %do  %while(&nut ne ); 
	%if &nut=DB020 %then %pepe111(&nut); 

	%if (&nut=NUTS1 or &nut=NUTS2) %then %do;
     	%if &yyyy > 2007 and (&Ucc=CZ or &Ucc=DK  or &Ucc=IE or &Ucc=ES or &Ucc=IT or
		&Ucc=SK or &Ucc=FI or &Ucc =SE or  &Ucc =BG or &Ucc  =SI or &Ucc  =RO)  %then %pepe111(&nut) ;
	 	%if &yyyy < 2008
		and  (&Ucc=CZ  or &Ucc=IE or &Ucc=ES or &Ucc=IT or
		&Ucc=SK or &Ucc=FI ) %then %pepe111(&nut) ;
		%if  &yyyy =2007 and (&Ucc= RO or &Ucc= DK) %then %pepe111(&nut) ;
		%if &yyyy >2004 and &Ucc=NO and &nut=NUTS1 %then %pepe111(&nut) ;
		%if  &Ucc=NO and &nut=NUTS2 and  &yyyy ne 2004 %then %pepe111(&nut) ;
		%if  &Ucc=CH and &nut=NUTS2 and  &yyyy > 2007 %then %pepe111(&nut) ;
	%end;
   %if &nut=NUTS1 and  &Ucc=PL and  &yyyy > 2011 %then %pepe111(&nut) ;
	%if &nut=NUTS1 and  (&Ucc=HU or &Ucc=EL /*or &Ucc=BE*/ ) %then %pepe111(&nut) ;
	%if &nut=NUTS1 and  &Ucc=NL and &yyyy > 2010 %then %pepe111(&nut) ;
	/*%if &nut=NUTS2 and  &Ucc=AT and &yyyy > 2007 and &yyyy < 2013 %then %pepe111(&nut) ;*/
	%let i=%eval(&i+1);                                  
	%let nut=%scan(&listnut,&i,%str( )); 
  %end;  

%end;
/* insert the  esport in csv file: ExportCsv   
expcsv;*/
%mend UPD_peps11; 
