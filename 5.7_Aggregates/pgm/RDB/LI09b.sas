/* LI09 UPDATED for EP question: At-risk-poverty-rate by type of household  */

*** At-risk-poverty-rate by age and gender before social transfers and pension***;

%macro UPD_li09b(yyyy,Ucc,Uccs,flag) /store;
...
Create table work.idb as 
	select DB010, DB020, DB030, RB030, RB050a,  EQ_INC23,HT1,
		(case
	    when EQ_INC23 < ARPT60 then 1
		else 0
		end) as ARPT60i23 
	from idb.IDB&yy
	where  DB010 = &yyyy and DB020 in &Uccs and HT1 between 1 and 13;
...
%if &EU=0 %then %do;
	...
	CLASS ARPT60i23;
	Class DB020;
	TABLE  DB020*HT1, ARPT60i23 *(RB050a * (RowPctSum Sum N)) /printmiss;
	RUN;
