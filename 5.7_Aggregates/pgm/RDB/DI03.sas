*** Mean and median income by age and gender  ***;
%macro UPD_DI03(yyyy,Ucc,Uccs,flag) /store;
/*20122602MG CREATED from LI02 TO MAKE THE AGGREGATES CALCULATIONS */
...
Create table work.idb as 
	select DB010, DB020, DB030, RB030, RB050a,  Age, RB090, EQ_INC20, rate, ppp
...
/* PROCESS TO APPEND  MEANINC value after MEDIANINC  inserting the UNIT variable  */	
		data mm(drop=_freq_ _type_);set mm;where _type_=3;run;
...
/* END APPEND PROCESS */  
	proc sql;
 	INSERT INTO &tab SELECT 
		...
		AND (TotValue.RB090 = old_flag.sex) and (TotValue.indic_il = old_flag.indic_il) and  (TotValue.unit = old_flag.unit);
...
PROC SQL;  
     ...
     set date = "&sysdate"d, time = "&systime"t, user = "&sysuserid",
		 report = "* &Ucc - &yyyy * DI03 (re)calculated *";		  
QUIT;
