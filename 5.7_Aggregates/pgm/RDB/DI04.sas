...
/*20122602MG CREATED from LI03 TO MAKE THE AGGREGATES CALCULATIONS */
%macro UPD_DI04(yyyy,Ucc,Uccs,flag) /store;
...
Create table work.idb as 
	select DB010, DB020, DB030, RB030, RB050a,  Age, RB090, HT, EQ_INC20, 
			rate, ppp   
	from idb.IDB&yy
...
%if &EU=0 %then %do;
	...
	data md (drop=MEDIANINC rate ppp rename=(MEANINC=ivalue));set mm1;
		...
		UNIT="NAC";
	run;
	data md;format DB010 DB020 HT INDIC_IL UNIT ivalue;
						set md;run;
	...
	data mde (drop=MEDIANINC MEANINC rate ppp rename=(MEANINCe=ivalue) );set mm1;
		...
		UNIT="EUR";
	run;
	data mde;format DB010 DB020 HT INDIC_IL UNIT ivalue;
						set mde;run;
	...
	data mdp;format DB010 DB020 HT INDIC_IL UNIT ivalue;
						set mdp;run;
...
/* define the indic_il   for median  and calculation of MEANINC for EUR and PPS  */
	...
	data memd;format DB010 DB020 HT INDIC_IL UNIT ivalue;
		set memd;run;
	...
	data memde (drop=MEANINC MEDIANINC rate ppp rename=(MEDIANINCe=ivalue) ) ;set mm1; 
		...
		UNIT="EUR";
	run; 
	data memde;format DB010 DB020 HT INDIC_IL UNIT ivalue;
	set memde;run;
	...
	data memdp (drop=MEANINC MEDIANINC rate ppp rename=(MEDIANINCp=ivalue) ) ;set mm1; 
		...
		UNIT="PPS";
	run; 
	data memdp;format DB010 DB020 HT INDIC_IL UNIT ivalue;
	set memdp;run;
...
/* END APPEND PROCESS */
  			Proc sql;
				...
				FROM TotValue  LEFT JOIN work.old_flag ON ("&Ucc" = old_flag.geo) AND (TotValue.HT = old_flag.hhtyp)
				AND (TotValue.indic_il = old_flag.indic_il)	and (TotValue.unit = old_flag.unit);
				QUIT;
...
PROC SQL;  
     ...
     set date = "&sysdate"d, time = "&systime"t, user = "&sysuserid",
		 report = "* &Ucc - &yyyy * DI04 (re)calculated *";		  
QUIT;
