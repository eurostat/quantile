*** Median income by age and gender and c_birth ***;
%macro UPD_DI16(yyyy,Ucc,Uccs,flag) /store;
...
Create table work.idb as 
	select DB010, DB020, DB030, RB030, RB050a,  Age, RB090, EQ_INC20, rate, ppp,C_BIRTH
...
group by time, geo /*,age */, sex, c_birth, unit ;
...
/* END APPEND PROCESS */  
	proc sql;
 	INSERT INTO &tab SELECT 
		...
		AND (TotValue.RB090 = old_flag.sex) and (TotValue.c_birth = old_flag.c_birth) and (TotValue.indic_il = old_flag.indic_il)  and  (TotValue.unit = old_flag.unit);
...
PROC SQL;  
     ...
     set date = "&sysdate"d, time = "&systime"t, user = "&sysuserid",
		 report = "* &Ucc - &yyyy * DI16 (re)calculated *";		  
QUIT;
