...
%macro UPD_peps05 (yyyy,Ucc,Uccs,flag,notBDB);
...
Create table work.idb as 
	select distinct DB010, DB020, RB030, RB050a, Age, RB090, AROPE, CIT_SHIP
...
CREATE TABLE missunrel AS SELECT mAGE.DB020, 
	max(mAGE, mRB090, mAROPE, mCIT_SHIP) AS pcmiss
	FROM mAGE 
...
PROC TABULATE data=work.idb out=Ti;
	...
	TABLE DB020 * AGE * RB090 * CIT_SHIP, AROPE * RB050a * (RowPctSum N Sum) /printmiss;
...
* Update RDB;
DATA  rdb.peps05 (drop= AROPE);
set rdb.peps05(where=(not(time = &yyyy and geo = "&Ucc" and unit = "&unit")))
    work.peps05;
	where AROPE="1";
RUN;
%mend by_unit;
...
PROC SQL;  
     ...
     set date = "&sysdate"d, time = "&systime"t, user = "&sysuserid",
		 report = "* &Ucc - &yyyy * peps05 (re)calculated *";		  
QUIT;
