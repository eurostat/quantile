...
%macro UPD_li22b(yyyy,Ucc,Uccs,flag) /store;
...
%if &any_year <= &yyyy    %then %do;
    ...
    %if  &nobs >0 %then %do;
		PROC SQL;  
			...
			report = "* &Ucc - &yyyy * li22b (re)calculated *";		  
		QUIT;
	%end;
    %else %do;
		PROC SQL;  
...
%else %do;
	...
	PROC SQL;  
		Insert into log.log
			set date = "&sysdate"d, time = "&systime"t, user = "&sysuserid",
		report = "* &Ucc - &yyyy <  reference year  for li22b *";		  
	QUIT;
