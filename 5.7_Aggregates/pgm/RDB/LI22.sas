...
%macro UPD_li22(yyyy,Ucc,Uccs,flag) /store;
...
%macro any_year (any_year);
	...
	%then %do;
		...
		FROM Ti 
			LEFT JOIN work.old_flag ON (Ti.DB020=old_flag.geo) AND ( Ti.RB090 = old_flag.sex)
		GROUP BY Ti.DB020, ti.AGE, ti.RB090
		ORDER BY Ti.DB020, ti.AGE, ti.RB090;
		QUIT;

		* Update RDB;
		DATA  rdb.li22 (drop= ARPT60ix);
