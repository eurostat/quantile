%macro UPD_lvho15(yyyy,Ucc,Uccs,flag,notBDB);
* lvho15 Overcrowding rate by age gender and citizen
...
CREATE TABLE missunrel AS SELECT mOVERCROWDED.DB020, 
                                 max(mOVERCROWDED, mCIT_SHIP) AS pcmiss
	FROM mOVERCROWDED
	LEFT JOIN mCIT_SHIP ON (mOVERCROWDED.DB020 = mCIT_SHIP.DB020)
	;;
QUIT;
...
PROC TABULATE data=work.idb out=Ti;
		...
		CLASS OVERCROWDED;
		VAR RB050a;
	TABLE DB020 * AGE * RB090*CIT_SHIP , OVERCROWDED * RB050a * (RowPctSum N Sum) /printmiss;
RUN;
