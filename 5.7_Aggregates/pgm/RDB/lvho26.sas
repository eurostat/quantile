%macro UPD_lvho26(yyyy,Ucc,Uccs,flag,notBDB);
...
/*Housing cost burden - by age sex and c_birth - population greater than 17 */
/* created on 20130724MG  */
...
%if &EU=0 %then %do;
	...
	*calculate HCB (housing cost burden);
	...
	PROC SQL;
	 CREATE TABLE work.idb AS SELECT DISTINCT IDB.DB010,IDB.DB020, IDB.RB030, IDB.RB050a, IDB.AGE,idb.c_birth,
...
* calculate % missing values;
	...
	CREATE TABLE missunrel AS SELECT mHCB.DB020, 
                                 max(mHCB, mc_birth) AS pcmiss
	FROM mHCB
	LEFT JOIN mc_birth ON (mHCB.DB020 = mc_birth.DB020)
	;
	QUIT;
	...
	* calc values, N and total weights;
	...
	PROC TABULATE data=work.idb out=Ti;
		...
		VAR RB050a;
		TABLE   DB020 * AGE * RB090 * c_birth, HCB * RB050a * (RowPctSum N Sum) /printmiss;
	RUN;
...
* EU aggregates;
	...
	PROC SQL;  
		 ...
		 report = "* &Ucc - &yyyy * lvho26 (re)calculated *";		  
	QUIT;
...
%else %do;
	PROC SQL;  
	     Insert into log.log
	     set date = "&sysdate"d, time = "&systime"t, user = "&sysuserid",
		 report = "* &Ucc - &yyyy * NO lvho26 CALCULATION ALLOWED FOR *";		  
	QUIT;
%end;
%let not60=0;
%mend UPD_lvho26;
