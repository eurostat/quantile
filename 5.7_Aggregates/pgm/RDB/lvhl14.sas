/*
. LWI by age sex and isced level 
*/

/* version of 16/11/2010*/
/*20120601MG changed ISCED code */

%macro UPD_lvhl14(yyyy,Ucc,Uccs,flag,notBDB);
...
Create table work.idb as 
	select distinct DB010, DB020, RB030, PB040, Age, RB090, LWI, PE40
...
CREATE TABLE missunrel AS SELECT mAGE.DB020, 
	max(mAGE, mRB090, mLWI, mPE40) AS pcmiss
...
PROC TABULATE data=work.idb out=Ti;
	...
	TABLE DB020 * AGE * RB090 * PE40, LWI * PB040 * (RowPctSum N Sum) /printmiss;
...
FROM Ti LEFT JOIN missunrel ON (Ti.DB020 = missunrel.DB020)
		LEFT JOIN work.old_flag ON (Ti.DB020 = old_flag.geo) AND (Ti.Age = old_flag.Age) 
				AND (Ti.RB090 = old_flag.sex) AND (Ti.PE40 = old_flag.isced97)
GROUP BY Ti.DB020, ti.AGE, ti.RB090, Ti.PE40
ORDER BY Ti.DB020, ti.AGE, ti.RB090, Ti.PE40;
QUIT;

* Update RDB;
DATA  rdb.lvhl14 (drop= LWI);
set rdb.lvhl14(where=(not(time = &yyyy and geo = "&Ucc" and unit = "&unit")))
    work.lvhl14;
	where LWI=1;
RUN;
%mend by_unit;
...
PROC SQL;  
     ...
     set date = "&sysdate"d, time = "&systime"t, user = "&sysuserid",
		 report = "* &Ucc - &yyyy * lvhl14 (re)calculated *";		  
QUIT;
