*** At-risk-poverty-rate by age and gender before social transfers excluding pensions***;
...
%macro UPD_li10(yyyy,Ucc,Uccs,flag) /store;
...
Create table work.idb as 
	select DB010, DB020, DB030, RB030, RB050a, Age, RB090, EQ_INC22,
		(case
	    when EQ_INC22 < ARPT60 then 1
		else 0
		end) as ARPT60i22, 
		(case
	    when EQ_INC22 < ARPT40 then 1
		else 0
		end) as ARPT40i22, 
		(case
	    when EQ_INC22 < ARPT50 then 1
		else 0
		end) as ARPT50i22, 
		(case
	    when EQ_INC22 < ARPT70 then 1
		else 0
		end) as ARPT70i22, 
		(case
	    when EQ_INC22 < ARPT60M then 1
		else 0
		end) as ARPT60Mi22, 
		(case
	    when EQ_INC22 < ARPT40M then 1
		else 0
		end) as ARPT40Mi22, 
		(case
	    when EQ_INC22 < ARPT50M then 1
		else 0
		end) as ARPT50Mi22, 
		EQ_INC22eur, rate, ppp    
...
Select distinct count(DB010) as N 
	into :nobs
	from  work.idb;
Create table work.li10 like rdb.li10; 
QUIT;
...
%if &EU=0 %then %do;
				...
				LEFT JOIN work.old_flag ON ("&Ucc" = old_flag.geo) 
						 AND (Ti.RB090 = old_flag.sex)  and ("&libel" = old_flag.indic_il)
				WHERE &arpt=1;
				QUIT;

				%mend f_li10;

%f_li10(ARPT60i22,LI_R_MD60BT);
%f_li10(ARPT40i22,LI_R_MD40BT);
%f_li10(ARPT50i22,LI_R_MD50BT);
%f_li10(ARPT70i22,LI_R_MD70BT);
%f_li10(ARPT60Mi22,LI_R_M60BT);
%f_li10(ARPT40Mi22,LI_R_M40BT);
%f_li10(ARPT50Mi22,LI_R_M50BT);

* Update RDB; 
DATA  rdb.LI10;
set rdb.LI10(where=(not(time = &yyyy and geo = "&Ucc")))
    work.li10; 
run; 
%end;
...
PROC SQL;  
     ...
     set date = "&sysdate"d, time = "&systime"t, user = "&sysuserid",
		 report = "* &Ucc - &yyyy * LI10 (re)calculated *";		  
QUIT;
