...
%macro UPD_di09(yyyy,Ucc,Uccs,flag) /store;
...
Create table work.idb as 
	select DB010, DB020, DB030, RB030,  RB050a,  Age, RB090, TENSTA, EQ_INC20, 
			rate, ppp     
	from idb.IDB&yy
...
/* PROCESS TO APPEND  MEANINC value after MEDIANINC  inserting the UNIT variable  */
	...
	data md (drop=MEDIANINC rate ppp rename=(MEANINC=ivalue));set mm1;
		...
		UNIT="NAC";
	run;
	data md;format DB010 DB020 AGE RB090  TENSTA INDIC_IL UNIT ivalue;
						set md;run;
	...
	data mde (drop=MEDIANINC MEANINC rate ppp rename=(MEANINCe=ivalue) );set mm1;
		...
		UNIT="EUR";
	run;
	data mde;format DB010 DB020 AGE RB090  TENSTA INDIC_IL UNIT ivalue;
						set mde;run;
	...
	data mdp;format DB010 DB020 AGE RB090  TENSTA INDIC_IL UNIT ivalue;
						set mdp;run;
...
/* define the indic_il   for median  and calculation of MEANINC for EUR and PPS  */
	...
	data memd;format DB010 DB020 AGE RB090  TENSTA INDIC_IL UNIT ivalue;
		set memd;run;
	...
	data memde (drop=MEANINC MEDIANINC rate ppp rename=(MEDIANINCe=ivalue) ) ;set mm1; 
		...
		UNIT="EUR";
	run; 
	data memde;format DB010 DB020 AGE RB090  TENSTA INDIC_IL UNIT ivalue;
	set memde;run;
	...
	data memdp (drop=MEANINC MEDIANINC rate ppp rename=(MEDIANINCp=ivalue) ) ;set mm1; 
		...
		UNIT="PPS";
	run; 
	data memdp;format DB010 DB020 AGE RB090  TENSTA INDIC_IL UNIT ivalue;
	set memdp;run;
...
/* END APPEND PROCESS */
	proc sql;
				...
				FROM TotValue LEFT JOIN work.old_flag ON ("&Ucc" = old_flag.geo) AND (TotValue.TENSTA = old_flag.TENURE)
...
PROC SQL;  
     ...
     set date = "&sysdate"d, time = "&systime"t, user = "&sysuserid",
		 report = "* &Ucc - &yyyy * DI09 (re)calculated *";		  
QUIT;
