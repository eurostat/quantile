...
/* version of 26/11/2010*/
%macro UPD_li32 (yyyy,Ucc,Uccs,flag,notBDB) /store;
...
Create table work.idb as 
	select distinct DB010, DB020, RB030, RB050a, Age, RB090, ARPT60i, C_BIRTH
...
CREATE TABLE missunrel AS SELECT mAGE.DB020, 
	max(mAGE, mRB090, mARPT60i, mC_BIRTH) AS pcmiss
	FROM mAGE 
...
PROC TABULATE data=work.idb out=Ti;
	...
	TABLE DB020 * AGE * RB090 * C_BIRTH, ARPT60i * RB050a * (RowPctSum N Sum) /printmiss;
...
* Update RDB;
DATA  rdb.li32 (drop= ARPT60i);
set rdb.li32(where=(not(time = &yyyy and geo = "&Ucc" and unit = "&unit")))
    work.li32;
	where ARPT60i=1;
RUN;
%mend by_unit;
...
PROC SQL;  
     ...
     set date = "&sysdate"d, time = "&systime"t, user = "&sysuserid",
		 report = "* &Ucc - &yyyy * li32 (re)calculated *";		  
QUIT;
