...
%macro UPD_lvhl17(yyyy,Ucc,Uccs,flag,notBDB);
...
Create table work.idb as 
	select distinct DB010, DB020, RB030, PB040, LWI, TENSTA_2,RB050a
...
CREATE TABLE missunrel AS SELECT mTENSTA_2.DB020, 
	max(mTENSTA_2, mLWI) AS pcmiss
	FROM mTENSTA_2 	LEFT JOIN mLWI ON (mTENSTA_2.DB020 = mLWI.DB020);
QUIT;
...
PROC TABULATE data=work.idb out=Ti;
	...
	TABLE DB020 * TENSTA_2 , LWI * RB050a * (RowPctSum N Sum) /printmiss;
...
* Update RDB;
DATA  rdb.lvhl17 (drop= LWI);
set rdb.lvhl17(where=(not(time = &yyyy and geo = "&Ucc" and unit = "&unit")))
    work.lvhl17;
	where LWI=1;
RUN;
%mend by_unit;

	%by_unit(PC_Y_LT60,RB050a_PctSum_101);
...
PROC SQL;  
     ...
     set date = "&sysdate"d, time = "&systime"t, user = "&sysuserid",
		 report = "* &Ucc - &yyyy * lvhl17 (re)calculated *";		  
QUIT;
